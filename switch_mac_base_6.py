#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @brief:     switch_mac_base_6.py - 测试交换机批量删除指定端口下动态地址
...
def get_random_mac():
    mac = "00." + '{0:0>2}'.format(hex(random.randint(0, 0xff))[2:]) + "." + \
          '{0:0>2}'.format(hex(random.randint(0, 0xff))[2:]) + ".00.00.11"
    return mac
...
def switch_mac_base_6(cb_arg):
    ...
    print "clear port mac before"
    time.sleep(30)
    sw.show_mac("count")
    sw.show_mac("count")

    # 清空指定port下的动态地址
    sw.clear_addr_entry("interface ", "GigabitEthernet ", active_intf[1])
    # 查看有无表项
    print "clear port mac later"
    sw.show_mac("count")
    sw.show_mac("count")
    time.sleep(5)

    print "clear port addr test is over"
    return result
